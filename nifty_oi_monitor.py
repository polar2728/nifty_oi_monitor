...
# NEW: Volume filter for execution signals
MIN_VOLUME_MULTIPLIER = 1.8  # Require volume at least 1.8× baseline
...
def scan():
    ...
    for _, r in df.iterrows():
        ...
        vol_multiplier = vol / base_vol if base_vol > 0 else 0
        ...
        if is_aggressive_writing:
            ...
            if vol_multiplier <= MIN_VOLUME_MULTIPLIER:
                print(f"Low volume skip at {strike} {opt}: {vol_multiplier:.1f}x <= {MIN_VOLUME_MULTIPLIER}")
                continue

            # === DUAL DECLINE CHECK (UPDATED) ===
            ...
            if opp_entry:
                ...
                # Check if opposite side data is available
                if opp_current_oi == 0:
                ...
                opp_cumulative_decline = ((opp_current_oi - opp_baseline_oi) / opp_baseline_oi * 100) if opp_baseline_oi > 0 else 0
                ...
                already_unwound = opp_cumulative_decline <= MIN_CUMULATIVE_DECLINE_PCT

                is_covering = scan_to_scan_covering or already_unwound

                if not is_covering:
                    print(f"⚠️ Rejected {strike} {opt}: opposite {opp_opt} not covering")
                    print(f"   Scan-to-scan: {opp_decline_pct:+.2f}% (need <= {MIN_DECLINE_PCT}%)")
                    print(f"   Cumulative: {opp_cumulative_decline:+.2f}% (need <= {MIN_CUMULATIVE_DECLINE_PCT}%)")
                    continue

                # Use the more significant decline for display
                opp_decline_display = min(opp_decline_pct, opp_cumulative_decline)

                # Debug print when covering detected
                print(f"✓ Covering detected at {strike} {opt}: {opp_opt} {opp_decline_display:.1f}% "
                      f"(scan-to-scan: {opp_decline_pct:+.2f}%, cumulative: {opp_cumulative_decline:+.2f}%) "
                ...
                buildup_info = {
                    ...
                    "vol_multiplier": round(vol_multiplier, 1),
                    "opp_decline_pct": opp_decline_display
                }
    ...
    # ================= Send grouped alerts =================
    if ce_buildups:
        # Sort by OI% and pick the strongest signal
        ce_buildups_sorted = sorted(ce_buildups, key=lambda x: x["oi_pct"], reverse=True)
        best = ce_buildups_sorted[0]
        trade_strike = best["strike"]
        trade_opt = "PE"
        details = "\n".join(
            f"{b['strike']} CE: +{b['oi_pct']:.0f}% | Vol {b['vol_multiplier']}x | PE {b['opp_decline_pct']:.1f}%"
            for b in ce_buildups_sorted[:5]  # Show top 5 max
        )
        msg = (
            f"🚀 *EXECUTION SIGNAL - CE BUILDUP*\n"
            f"*Action: Buy {trade_strike} {trade_opt}*\n\n"
            f"High-quality signals (PE unwinding):\n{details}\n\n"
            f"Spot: {spot}"
        )
    ...
    if pe_buildups:
        # Sort by OI% and pick the strongest signal
        pe_buildups_sorted = sorted(pe_buildups, key=lambda x: x["oi_pct"], reverse=True)
        best = pe_buildups_sorted[0]
        trade_strike = best["strike"]
        trade_opt = "CE"
        details = "\n".join(
            f"{b['strike']} PE: +{b['oi_pct']:.0f}% | Vol {b['vol_multiplier']}x | CE {b['opp_decline_pct']:.1f}%"
            for b in pe_buildups_sorted[:5]  # Show top 5 max
        )
        msg = (
            f"🚀 *EXECUTION SIGNAL - PE BUILDUP*\n"
            f"*Action: Buy {trade_strike} {trade_opt}*\n\n"
            f"High-quality signals (CE unwinding):\n{details}\n\n"
            f"Spot: {spot}"
        )
        send_telegram_alert(msg)
